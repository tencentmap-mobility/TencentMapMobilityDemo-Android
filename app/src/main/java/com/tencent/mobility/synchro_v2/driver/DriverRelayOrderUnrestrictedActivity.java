...
/**
 * 接力单功能：主订单和接力单可为快车类型和拼车类型
 */
public class DriverRelayOrderUnrestrictedActivity extends BaseActivity {
    ...
    private void initCurrentPassengerPanel() {
        ...
        mPassengerSyncA.init(DriverRelayOrderUnrestrictedActivity.this,
        ...
        mPassengerAPanel.init("当前乘客", "创建订单-快+快", "创建订单-快+拼", "创建订单-拼+快");
        mPassengerSyncA.setPullTimeInterval(3);
        mPassengerAPanel.addAction("创建订单-快+快", new PanelView.Action<String>("") {
            @Override
            public String run() {
                ...
                if (order != null && !TextUtils.isEmpty(order.getId())) {
                    ...
                    mDriverPanel.postAction("绑定订单-快车");
                    mPassengerBPanel.postAction("创建接力订单-快车");
                    return order.getId();
                }
                return "";
            }
        ...
        mPassengerAPanel.addAction("创建订单-快+拼", new PanelView.Action<String>("") {
            @Override
            public String run() {
                mPassengerAPanel.postAction("开启同显");
                mPassengerA.setPosition(MockSyncService.getRandomVisibleLatLng(mMapView1.getMapApi().getProjection()));
                ...
                if (order != null && !TextUtils.isEmpty(order.getId())) {
                    ...
                    mPassengerSyncA.getOrderManager().editCurrent()
                            ...
                            .setOrderId(order.getId());
                    mDriverPanel.postAction("绑定订单-快车");
                    mPassengerBPanel.postAction("创建接力订单-拼车");
                    return order.getId();
                }
                return "";
            }
        ...
        mPassengerAPanel.addAction("创建订单-拼+快", new PanelView.Action<String>("") {
            @Override
            public String run() {
                mPassengerAPanel.postAction("开启同显");
                mPassengerA.setPosition(MockSyncService.getRandomVisibleLatLng(mMapView1.getMapApi().getProjection()));
                ...
                if (order != null && !TextUtils.isEmpty(order.getId())) {
                    ...
                    requestMap.put("driver_orderid", order.getId());
                    requestMap.put("passenger_orderid", order.getId() + "-A");
                    ...
                    mDriverPanel.postAction("绑定订单-拼车");
                    mPassengerBPanel.postAction("创建接力订单-快车");
                    return order.getId();
                }
                return "";
    ...
    private void initRelayPassengerPanel() {
        ...
        mPassengerSyncB.init(DriverRelayOrderUnrestrictedActivity.this,
        ...
        mPassengerBPanel.addAction("创建接力订单-快车", new PanelView.Action<String>("") {
            @Override
            public String run() {
                ...
                if (order != null && !TextUtils.isEmpty(order.getId())) {
                    ...
                    mPassengerSyncB.getOrderManager().editCurrent()
                            .setOrderStatus(TLSBOrderStatus.TLSDOrderStatusInit)
                            .setOrderId(order.getId());
                    mDriverPanel.postAction("绑定接力订单-快车");
                    return order.getId();
                }
                return "";
            }
        ...
        mPassengerBPanel.addAction("创建接力订单-拼车", new PanelView.Action<String>("") {
            @Override
            public String run() {
                mPassengerBPanel.postAction("开启同显");
                mPassengerB.setPosition(MockSyncService.getRandomVisibleLatLng(mMapView2.getMapApi().getProjection()));
                ...
                if (order != null && !TextUtils.isEmpty(order.getId())) {
                    ...
                    mPassengerSyncB.getOrderManager().editCurrent()
                            ...
                            .setOrderType(TLSBOrderType.TLSBOrderTypeRidesharing)
                            .setOrderId(order.getId())
                            .setSubOrderId(order.getCarpoolOrderId() + "-B");
                    mDriverPanel.postAction("绑定接力订单-拼车");
                    return order.getId();
                }
                return "";
            }
        ...
        mPassengerBPanel.addAction("开启同显", new PanelView.Action<Boolean>(false) {
            ...
            public Boolean run() {
                mPassengerSyncB.addTLSPassengerListener(new SimplePsgDataListener() {
                    ...
                    public void onPullLsInfoSuc(TLSDFetchedData fetchedData) {
                        if (fetchedData != null
//                                && mPassengerSyncB.getOrderManager().getOrderStatus() == 2
                        ) {
    ...
    private void initDriverPanel() {
        ...
        mDriverSync.init(DriverRelayOrderUnrestrictedActivity.this,
        ...
        mDriverPanel.addAction("绑定订单-快车", new PanelView.Action<String>("") {
            @Override
            public String run() {
                mDriverSync.start();
        ...
        mDriverPanel.addAction("绑定订单-拼车", new PanelView.Action<String>("") {
            @Override
            public String run() {
                mDriverSync.start();
                mDriver.setPosition(MockSyncService.getRandomVisibleLatLng(mCarNaviView.getMapApi().getProjection()));
        ...
        mDriverPanel.addAction("绑定接力订单-快车", new PanelView.Action<String>("") {
            @Override
            public String run() {
        ...
        mDriverPanel.addAction("绑定接力订单-拼车", new PanelView.Action<String>("") {
            @Override
            public String run() {
                MockOrder order = mDriverSyncService.getOrder(mPassengerB);
                mPassengerSyncB.getOrderManager().editCurrent().setOrderStatus(TLSBOrderStatus.TLSDOrderStatusPickUp);
                ...
                requestMap.put("type", 4);
                requestMap.put("driver_orderid", order.getId());
                requestMap.put("passenger_orderid", order.getId() + "-B");
                mDriverSync.orderStatusSync(order.getId(), requestMap, new SyncProtocol.OrderResultListener() {
                    @Override
                    public void onResult(int status, String message) {
                        mDriverPanel.print("接驾接力单同步, status: " + status + ", message: " + message);
